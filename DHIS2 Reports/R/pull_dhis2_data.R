...
options(dhis2_username = cred$username)
options(dhis2_password = cred$password)
options(dhis2_base = "https://hmis.health.go.ug/api/")

## Tables
loc_table <-  fread("tables/location_hierarchy.csv")
indicator_table <- fread("tables/dhis_indicator_table.csv")
...
pull_reporting_rates <- function(weeks) {
  ...
  url <- paste0("https://hmis.health.go.ug/api/29/analytics/dataValueSet.json?dimension=dx:C4oUitImBPK.REPORTING_RATE&dimension=pe:", pe_string, "&dimension=ou:akV6429SUqu&displayProperty=NAME")
  data <- pull_url(url)
  return(data)
}

pull_district_monthly_age_specific <- function(months) {
  age_sex_table <- fread("tables/age_sex_table.csv")
